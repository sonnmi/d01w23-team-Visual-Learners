...
A PatchCollection is a container that allows users to group patch elements together. A legend is used as a reference to understand and identify what each element of a plot is supposed to be. Therefore, the question arises: why does legend not support PatchCollection? `Legend` supports other collections already, from `PathCollection` to `PolygonCollection`, so `PatchCollection` will fit right in.

To allow the `Legend` class to support `PatchCollection` objects, we need to do the following:
-   Create a new subclass that inherits from `HandlerBase` (or subclasses that eventually inherit HandlerBase) - call it `HandlerPatchCollection`
```python
# matplotlib-main/lib/matplotlib/legend_handler.py line 819
class HandlerPatchCollection(HandlerPatch):
	...
```
-   Override `HandlerBase`’s `create_artists()` function along with other functions that deem necessary - taking inspiration from `HandlerPatch`, `HandlerPolyCollection`, and `HandlerLineCollection`
```python
...
def create_artists(self, legend, orig_handle, 
	...
	...
```
-   Update the `_default_handler_map` in `Legend` class to include this new subclass - in our case, mapping `PatchCollection` to `HandlerPatchCollection`
```python
...
-   lib/matplotlib/tests/test_legend.py - to add unit and acceptance tests for HandlerPatchCollection (starts at line 1267 first four are acceptance, next five are unit tests)

This part of the issue led itself in being straightforward and no such changes were made to the design of Matplotlib. 
