...
-   Create a new subclass that inherits from `HandlerBase` (or subclasses that eventually inherit HandlerBase) - let's call it `HandlerPatchCollection`
```python
# matplotlib-main/lib/matplotlib/legend_handler.py line 819
class HandlerPatchCollection(HandlerPatch):
	...
```
-   Override `HandlerBase`’s `create_artists()` function, adding other functions that may be necessary - taking inspiration from `HandlerPatch`, `HandlerPolyCollection`, and `HandlerLineCollection`
```python
